...
const CommentsSection = ({ comments, commentIsClicked, whoLikedState, postId }) => {
  ...
  const whoLikedThat = () => {
    ...
    if (whoLikedState.length > 3) usersWhoLikedStr += `and ${whoLikedState.length - 2} others `;
  ...
  const sendingNewComment = async e => {
    ...
    await fetch(`https://instagram-database.youngwebdevs.now.sh/feed/comment?_id=${postId}`, putMethod);
  };

  const deleteComment = async e => {
    const uniqueKeyClicked = e.target.id;
    const arrayAfterRemove = commentsArray.filter(comment => comment.uniqueKey !== uniqueKeyClicked);
