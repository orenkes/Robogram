...
const Likes = ({ likedBy, postId, setWhoLikedState }) => {
  const [youLikedIt, setYouLikedIt] = useState(false);
  const [fiktzia, setFiktzia] = useState(false);
  ...
  const likePost = async () => {
    ...
    if (youLikedIt === false) {
      ...
      // console.log("liked by state: ", likedByState);
    } else {
    ...
    setWhoLikedState(jsonPost.likedBy);
  };

  return (
    <LikesBox onClick={() => likePost()}>
      <StyledFiHeart youLikedIt={youLikedIt} fiktzia={fiktzia} />
    </LikesBox>
  );
};
...
const StyledFiHeart = styled(FiHeart)`
  fill: ${({ fiktzia }) => (fiktzia ? "crimson" : "white")};
  fill: ${({ youLikedIt }) => (youLikedIt ? "crimson" : "white")};
  color: ${({ fiktzia }) => (fiktzia ? "crimson" : "black")};
